...
function sendTransactionProposal(channelName, chaincodeName, fcn, args){
...
function sendTransaction(results, peersUrls, channelName, chaincodeName, fcn, args, username, org) {
  ...
  if (!all_good && peerFailures < ordererCount) {
      ...
      return invokeChaincode(peersUrls, channelName, chaincodeName, fcn, args, username, org);
  }
  if (all_good) {
      if (ordererFailure < 1) {
...
function validateResponse(response, peersUrls, channelName, chaincodeName, fcn, args, username, org){
...
async function invokeChaincode (peersUrls, channelName, chaincodeName, fcn, args, username, org) {
    logger.debug(util.format('\n============ invoke transaction on organization %s ============\n', org));
    client = helper.getClientForOrg(org);
    channel = helper.getChannelForOrg(org, channelName);
    ...
    targets = helper.newPeers(peersUrls);
    ordererCount = channel.getOrderers().length;

    try {
      var user = await helper.getRegisteredUsers(username, org);
      try {
        var results = await sendTransactionProposal(channelName, chaincodeName, fcn, args);
      } catch (err) {
      ...
      try {
        var response = await sendTransaction(results, peersUrls, channelName, chaincodeName, fcn, args, username, org);
      } catch (err) {
      ...
      return await validateResponse(response, peersUrls, channelName, chaincodeName, fcn, args, username, org);
    } catch (err) {
